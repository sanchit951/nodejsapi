var express = require('express');
var path = require('path');
var favicon = require('serve-favicon');
var logger = require('morgan');
var cookieParser = require('cookie-parser');
var bodyParser = require('body-parser');
var cors = require('cors');
var index = require('./routes/index');
var users = require('./routes/users');
var employeeFetch = require('./routes/FEtchEmployee');
var editEmployee = require('./routes/EditEmployee');
var deleteEmployee = require('./routes/DeleteEmployee');
var addEmployee = require('./routes/AddEmployee');
var advanceSearch = require("./routes/AdvanceSearch");
var fetchDepartment=require("./routes/FetchDepartment");
var employeeDepartment=require('./routes/EmployeeDepartment');
var app = express();

// view engine setup
app.set('views', path.join(__dirname, 'views'));
app.set('view engine', 'jade');

// uncomment after placing your favicon in /public
//app.use(favicon(path.join(__dirname, 'public', 'favicon.ico')));
app.use(logger('dev'));
app.use(bodyParser.json());
app.use(bodyParser.urlencoded({ extended: false }));
app.use(cookieParser());
app.use(express.static(path.join(__dirname, 'public')));
app.use(cors());
app.use('/', index);
app.use('/users', users);
app.use('/fetchemployee', employeeFetch)
app.use('/editemployee', editEmployee)
app.use('/deleteEmployee', deleteEmployee);
app.use('/addemployee', addEmployee)
app.use('/advancesearch', advanceSearch)
app.use('/fetchDepartment',fetchDepartment);
app.use('/employeeDepartment',employeeDepartment);
// catch 404 and forward to error handler
app.use(function (req, res, next) {
  var err = new Error('Not Found');
  err.status = 404;
  next(err);
});

// error handler
app.use(function (err, req, res, next) {
  // set locals, only providing error in development
  res.locals.message = err.message;
  res.locals.error = req.app.get('env') === 'development' ? err : {};

  // render the error page
  res.status(err.status || 500);
  res.render('error');
});

module.exports = app;
